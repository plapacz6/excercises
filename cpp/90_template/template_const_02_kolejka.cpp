...
struct el_kolejki {
  TT& el;
  el_kolejki* next;
  el_kolejki(TT& el_) : el(el_), next(nullptr) {};
};
...
template <typename T>
kolejka<T>::~kolejka(){
  el_kolejki<T>* tmp;             //elementy w kolejnie nie zostaly zniszczone 
...
void kolejka<T>::push_back(T& x){
  ...
  if(first == nullptr) {
    last = new el_kolejki<T>(x);
    first = last;
  }
  else {
    last->next = new el_kolejki<T>(x);
    last = last->next;
  }
  dlugosc_kolejki++;
}
